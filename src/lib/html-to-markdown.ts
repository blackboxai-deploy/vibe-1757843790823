  ...
  rows.forEach((row, index) => {
    ...
    const cellContents = cells.map(cell => {
      ...
      cellContent = cellContent
        ...
        // Handle lists within cells - preserve bullet points
        .replace(/<ul[^>]*>([\s\S]*?)<\/ul>/gi, (match: string, listContent: string) => {
          ...
          const processedItems = listItems.map((item: string) => {
            ...
            return `• ${itemText}`; // Use bullet character
          }).filter(item => item.length > 2); // Filter out empty items (just "• ")
        ...
        .replace(/<ol[^>]*>([\s\S]*?)<\/ol>/gi, (match: string, listContent: string) => {
          ...
          }).filter(item => item.length > 3); // Filter out empty numbered items
          return processedItems.join(' ');
        })
        // Handle paragraphs
